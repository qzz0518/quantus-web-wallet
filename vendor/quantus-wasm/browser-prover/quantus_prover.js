/* @ts-self-types="./quantus_prover.d.ts" */
 ...
 * @returns {any}
 */
export function wormholeProveExit(mnemonic, request, on_progress) {
    const ptr0 = passStringToWasm0(mnemonic, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.wormholeProveExit(ptr0, len0, request, isLikeNone(on_progress) ? 0 : addToExternrefTable0(on_progress));
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
 ...
 * @returns {any}
 */
export function wormholeProverInfo() {
    const ret = wasm.wormholeProverInfo();
    if (ret[2]) {
        throw takeFromExternrefTable0(ret[1]);
    }
    return takeFromExternrefTable0(ret[0]);
 ...
 * @param {string} mnemonic
 * @param {number} count
 * @param {string} exit_address
 * @returns {string}
 */
export function wormholeSyntheticExitRequest(mnemonic, count, exit_address) {
    let deferred4_0;
    let deferred4_1;
    try {
        const ptr0 = passStringToWasm0(mnemonic, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ptr1 = passStringToWasm0(exit_address, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len1 = WASM_VECTOR_LEN;
        const ret = wasm.wormholeSyntheticExitRequest(ptr0, len0, count, ptr1, len1);
        var ptr3 = ret[0];
        var len3 = ret[1];
        if (ret[3]) {
            ptr3 = 0; len3 = 0;
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('quantus_prover_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
