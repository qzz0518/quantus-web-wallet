...
- The wallet screens talk only to the official Quantus RPC node and indexer, plus SafeTrade's public QUANTUS/USDT ticker once a minute for the dollar value of the balance; the mining calculator additionally reads Quanpool's public API, and the network dashboard reads `Balances.TotalIssuance` from the same RPC node and the indexer's public statistics tables (daily counters, global totals, block counts). The Content Security Policy served with the site allows no other host. There is no telemetry, no analytics and no service fee.
