...
- **Mining calculator.** Under Tools. Pick your GPUs (or enter a total hashrate), quantity and miner software, then uptime, pool and miner-software fees, electricity price or rig rent, an optional hardware cost, and the QTC price you assume. You get expected output per hour, day, week and 30 days, costs, profit and margin, cost per QTC, the break-even QTC price, payback time, a comparison of every known GPU at the current network state, and how the result moves if network hashrate grows 1.5× or 2×. Difficulty and measured block time come from the chain RPC, recent block rewards from the public indexer, and GPU benchmarks plus fee terms from Quanpool's public API (with a built-in snapshot as fallback). Everything is an expectation, QTC has no market price yet, and inputs stay in your browser.
...
- The page talks only to the official Quantus RPC node and indexer, enforced by the Content Security Policy served with the site. There is no telemetry, no analytics and no service fee.
